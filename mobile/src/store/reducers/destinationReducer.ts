...
// mobile/src/store/reducers/destinationReducer.ts
import { AnyAction } from 'redux';
...
export const FETCH_THINGS_TO_SEE_SUCCESS = 'FETCH_THINGS_TO_SEE_SUCCESS';
export const FETCH_THINGS_TO_SEE_FAILURE = 'FETCH_THINGS_TO_SEE_FAILURE';
export const DOCUMENT_VERIFIED_JOIN_GROUP = 'DOCUMENT_VERIFIED_JOIN_GROUP';
...
const destinationReducer = (
  ...
  switch (action.type) {
    ...
    case DOCUMENT_VERIFIED_JOIN_GROUP:
      // When a document is verified, automatically join the corresponding destination
      const { destinationId, subDestinationId, startDate, endDate } = action.payload;
      
      // Check if the destination exists
      const existingDestination = state.destinations.find(d => d.id === destinationId);
      ...
      if (existingDestination) {
          ...
          destinations: state.destinations.map(destination => {
            if (destination.id === destinationId) {
              return {
                ...
                memberCount: destination.memberCount + 1,
                subDestinations: destination.subDestinations.map(subDest => {
                  if (subDestinationId && subDest.id === subDestinationId) {
                    return {
                      ...subDest,
                      isJoined: true,
                      memberCount: subDest.memberCount + 1,
        ...
        // Add new temporary destination
        const newDestination: Destination = {
          id: destinationId,
          name: action.payload.destinationName,
